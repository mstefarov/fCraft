...
namespace fCraft.MapGeneration {
    ...
    internal sealed class RealisticMapGenState : MapGeneratorState {
        ...
        void GenerateHeightmap() {
            ...
            if (genParams.LayeredHeightmap) {
                ...
                new Noise(rand.Next(), NoiseInterpolationMode.Bicubic).PerlinNoise(heightmap2,
                                                                                   0,
                                                                                   genParams.DetailScale,
                                                                                   genParams.Roughness,
                                                                                   0,
                                                                                   0);
        ...
        void AddCaves([NotNull] Map map) {
            ...
            if (genParams.AddCaves) {
                ...
                for (int j1 = 0; j1 < 9*genParams.CaveDensity; j1++) {
                    AddSingleVein(rand,
                                  map,
                                  (byte)theme.BedrockBlock,
                                  (byte)Block.Air,
                                  500,
                                  0.015*genParams.CaveSize,
                                  1);
                }
            ...
            for (int x = 0; x < map.Width; x += rn.Next(minTrunkPadding, maxTrunkPadding + 1)) {
                for (int y = 0; y < map.Length; y += rn.Next(minTrunkPadding, maxTrunkPadding + 1)) {
                    ...
                    if ((map.GetBlock(nx, ny, nz) == theme.GroundSurfaceBlock) && slopeMap[nx, ny] < .5) {
                        ...
                        int nh;
                        if ((nh = rn.Next(minHeight, maxHeight + 1)) + nz + nh/2 > map.Height) continue;
                            ...
                            for (int xoff = -radius; xoff < radius + 1; xoff++) {
                                for (int yoff = -radius; yoff < radius + 1; yoff++) {
                                    ...
                                    if (rn.NextDouble() > odds && Math.Abs(xoff) == Math.Abs(yoff) &&
                                        Math.Abs(xoff) == radius) continue;
                                    // By default only replace an existing block if its air
                                    if (map.GetBlock(nx + xoff, ny + yoff, nz + nh + i) == Block.Air) map.SetBlock(nx + xoff, ny + yoff, nz + nh + i, Block.Leaves);
                                }
                            }
                        }
                    }
