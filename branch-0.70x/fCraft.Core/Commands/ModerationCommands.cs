...
namespace fCraft {
    ...
    static class ModerationCommands {
        ...
        static void RankHandler( Player player, CommandReader cmd ) {
            ...
            PlayerInfo targetInfo = PlayerDB.FindExact( name );
            ...
            if( targetInfo == null ) {
                ...
                if( cmd.IsConfirmed ) {
                    if( newRank > RankManager.DefaultRank ) {
                        targetInfo = PlayerDB.AddUnrecognizedPlayer( name, RankChangeType.Promoted );
                    } else {
                        targetInfo = PlayerDB.AddUnrecognizedPlayer( name, RankChangeType.Demoted );
                    }
                } else {
                    player.Confirm( cmd,
        ...
        static void SetSpawnHandler( Player player, CommandReader cmd ) {
            ...
            if( playerName == null ) {
                ...
                player.Send( Packet.MakeAddEntity( -1, player.ListName, player.Position ) );
                ...
                if( infos.Length == 1 ) {
                    ...
                    if( player.Can( Permission.Bring, target ) ) {
                        target.Send( Packet.MakeAddEntity( -1, target.ListName, player.Position ) );
                    } else {
